...
def Jac_s(Screws, theta):
    Js = np.zeros([6,np.size(Screws,axis=1)])
    M = np.eye((4))
...
def InvKinematics2(Screws, q_init, M_EF, T_desired):
    ...
    V = np.zeros((6,1))
    # Iteration algorithm
    while True: # LA.norm(T - T_desired, 'fro') > Tolerance:
        K = logm(T_desired.dot(Tinv(T)))
        # print(T_desired)
        V[0:3,0] = [K[2,1], K[0,2], K[1,0]]
        V[3:6,0] = K[0:3,3]
        delta_q = k * LA.pinv(Jac_s(Screws, q_iteration)).dot(V)
        q_iteration = q_iteration + delta_q
        q_iteration = Joint_Limit_Check(JointLimit()[1],JointLimit()[0],q_iteration)
        T = Screws_to_SE3(Screws, q_iteration).dot(M_EF)
        Iteration = Iteration + 1
        if LA.norm(V) < 1e-8:
            break
    ...
    for i in range(0,np.size(q_iteration,axis=0)):
          if q_iteration[i,0] > np.pi:
               q_iteration[i,0] -= 2* np.pi
...
def Joint_Limit_Check(ub,lb,joint):
    ...
    for i in range(np.size(joint)):
        if lb[i] <= joint[i] <= ub[i]:
            ...
            pass#print("joint is out of the limit")

    return joint

def Joint_Refine(ub,lb,joint):
    mp = (ub+lb)/2
    print(mp)
    for i in range(np.size(joint)): 
        dir=joint[i]-mp[i]
        dir=dir/abs(dir)
        joint_cand = joint[i] - np.pi *2 * dir
        if dir < 0 and ub[i]-joint_cand > joint[i] - lb[i]:
               joint[i] = joint_cand
        if dir > 0 and joint_cand-lb[i] > ub[i] - joint[i]:
               joint[i] = joint_cand
